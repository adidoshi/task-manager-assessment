...
<template>
  <div class="min-h-screen flex bg-canvas">
    <!-- Sidebar -->
    <aside
      class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-border-subtle flex flex-col shrink-0 transform transition-transform duration-200 ease-out lg:static lg:translate-x-0 lg:z-auto"
      :class="
    ...
    >
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="text-meta text-ink-400 truncate">
            adityadoshi25@gmail.com
          </div>
        </div>
        <button
          class="hidden lg:inline-flex w-7 h-7 rounded-md text-ink-400 hover:bg-canvas items-center justify-center"
        >
          <i class="fa-solid fa-ellipsis-vertical text-[12px]"></i>
        </button>
        <button
          class="lg:hidden w-7 h-7 rounded-md text-ink-400 hover:bg-canvas inline-flex items-center justify-center"
          @click="closeSidebar"
    ...
    <main class="flex-1 min-w-0 flex flex-col">
      <!-- Topbar -->
      <header
        class="h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between border-b border-border-subtle bg-white"
      >
        <div class="flex items-center gap-2 sm:gap-3 text-body-xs text-ink-500">
          <button
            class="lg:hidden w-8 h-8 rounded-lg hover:bg-canvas text-ink-500"
            @click="toggleSidebar"
          ...
          <span class="hidden sm:inline font-medium text-ink-500"
            >Team spaces</span
          >
          <i
            class="hidden sm:inline fa-solid fa-chevron-right text-[10px] text-ink-300"
          ></i>
          <span class="font-semibold text-ink-900">Tasks</span>
        </div>
        <div class="flex items-center gap-2 sm:gap-3">
          <div
            class="hidden xl:flex items-center gap-2 px-3 h-10 rounded-xl border border-border-subtle bg-canvas w-72"
          >
            <i
          ...
          <button
            class="hidden sm:inline-flex w-9 h-9 rounded-lg border border-border-subtle text-ink-500 hover:bg-canvas items-center justify-center"
          >
